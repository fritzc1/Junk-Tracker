...
| 5 | Attribute system (frontend) | ✅ Complete — revised per owner feedback into a dialog-based, per-item attribute model (single `ItemDialog.jsx` code path for create/view/edit + `AttributeEditor.jsx`; no attribute table columns); final cleanup removed the superseded entry page/form/picker dead code and `/entry`, `/edit/:id` routes. Verified 2026-09-05 on spare ports: item create with attribute, edit persistence, all main pages error-free; test data cleaned up, before/after counts identical |
| 6 | Attribute sets (type-scoped attribute profiles) | ✅ Complete — verified 2026-09-05 (spare-port API tests on a temp database, 36/36 checks: set CRUD + populated member dimensions, item validation with set scoping incl. out-of-set 400s naming attr+set, Stage 5 rev3 grandfathering preserved with a set assigned, bulk-assign reconciliation, rename touches no item data, delete guards with counts; real browser via Playwright on spare ports: resistor set → exactly its three pickers and nothing else, out-of-set attribute flagged "not in this set" + server rejection surfaced, Set column renders/filters/sorts, bulk-assign error surfaced; all test data cleaned up, before/after counts identical) |
...
| D7 | Box/location identity split: boxes are identified by a **required** `boxId` (unique per database) with NO user-facing name — `name` is auto-set to the boxId internally on every write; locations keep their required free-form `name`. Additionally, within a database the set of location names and the set of box IDs must be **disjoint** (case-insensitive). | Boxes are movable, so identity must be position-independent: the physical label is that stable key (and the import-dedup key). A free-text box name has no maintenance trigger — it drifts out of sync with contents and misleads. The disjointness rule keeps display paths unambiguous ("Garage / A06" can't mean both a location named A06 and box A06), which matters for CSV re-imports and human reading. Full spec: [container-box-identity-plan.md](./container-box-identity-plan.md). |
...
> **Revision note (2026-09-05):** Per owner feedback this stage was revised from the original design (one dropdown per dimension in `ItemEntryForm` + dynamic attribute columns on `ItemListPage`) into a **dialog-based, per-item attribute model**: all item create/view/edit flows through a single [`frontend/src/components/ItemDialog.jsx`](../frontend/src/components/ItemDialog.jsx) code path (Save/Cancel; Details section + divider + Attributes section), with per-item attribute rows edited by [`frontend/src/components/AttributeEditor.jsx`](../frontend/src/components/AttributeEditor.jsx) (prop-driven via `availableDimensions`). The items table has **no attribute columns**. Final cleanup removed the superseded full-page entry path — `pages/ItemEntryPage.jsx`, `components/ItemEntryForm.jsx`, `components/AttributePickers.jsx` — plus their `/entry` and `/edit/:id` routes in `App.jsx`; the canonical create path is now ItemListPage's "Add New Item" button opening ItemDialog in create mode. Re-verified 2026-09-05 on spare ports (backend 5099 + Vite 3100): item create with attribute, edit persistence, all main pages error-free; test data cleaned up, before/after counts identical.
...
- [x] Create `backend/models/AttributeSet.js`: `{ databaseId (required), name (unique per database, trim), attributeIds: [ObjectId ref Attribute] }`. Unique index on `(databaseId, name)`. *(pre-save hook trims the name and dedupes/drops invalid member ids; timestamps mirror the Attribute model)*
- [x] Update `backend/models/Item.js`: add nullable `attributeSetId` (ref `AttributeSet`). Purely additive — existing items simply have no set; no migration required. *(index on attributeSetId for the delete guard + per-set lookups)*
- [x] Extend item validation in the controller: when an item has a set, every key in `item.attributes` must belong to that set's dimensions and its value must be within that dimension's vocabulary; keys outside the set are rejected with a clear error. Items without a set keep Stage 4 behavior (any defined dimension allowed). *(exact shape: `Attribute "X" is not part of attribute set "Y". Remove it from the item or choose a different set.` — membership applies to ALL keys, so assigning a new set reconciles existing attributes; Stage 5 rev3 grandfathering preserved for unchanged key/value pairs. Verified: 400s name attr+set on create and update; no-set items still accept any defined dimension and still 400 on undefined dimensions)*
- [x] AttributeSet CRUD controller + routes: list / create / update / delete sets, edit their member dimensions; block deletion while items reference the set (return count); renaming a set touches no item data (items store the id). *(GET/POST/PUT/DELETE `/api/attribute-sets`, per active database via requireDatabase; list populates each set's member dimensions — name/dataType/unit/values — plus live itemCount; delete blocked with `Cannot delete attribute set "Y" — N item(s) still use it. Remove the set from those items first.` + `{ itemCount }`; rename verified to leave item refs untouched)*

### Step 6b: Set picker in UI
- [x] `ItemDialog.jsx` (the Stage 5 revision's unified dialog, replacing the original ItemEntryForm design): add an "Attribute set" dropdown at the top of the attribute section. Selecting a set passes that set's dimensions to the parameterized picker component from Step 5b; clearing it falls back to all defined dimensions (default behavior — decide exact fallback during implementation). *(fallback = ALL defined dimensions, exactly as Stage 5; helper text names the set's available dimensions. Attributes already on the item but not in the selected set stay visible, muted with a ⚠ + "not in this set" note so saving doesn't silently lose data; the server rejects them if submitted)*
- [x] `ItemListPage.jsx`: optional "Set" column showing each item's set name, filterable like other columns. *(blank when none; sortable via header click and offered as an advanced-search column — also fixed a latent Stage 5 bug where the advanced search crashed on selecting any column: `getSelectedColumnOption` referenced a removed `searchColumnOptions` variable)*
- [x] Bulk edit: allow assigning a set across selected items; server-side validation rejects attribute keys that fall outside the new set with an actionable error. *(Set dropdown in the Multi Edit dialog with a "Various — …" distribution caption; clearing removes the set from all selected items; per-item PUT loop surfaces the first rejection, e.g. `Error updating items: Attribute "color" is not part of attribute set "resistor". Remove it from the item or choose a different set.`)*

### Definition of Done — Stage 6
- [x] Define a "resistor" set (footprint + tolerance + value) → create an item with that set → form shows exactly those three pickers and nothing else. *(verified in browser on spare ports: Add-attribute pool contained only footprint/tolerance/value; the fourth defined dimension was absent)*
- [x] Saving an attribute key outside the item's set is rejected with a clear error; items without a set still behave as in Stage 5. *(in-browser save of an out-of-set attribute surfaced `Attribute "color" is not part of attribute set "resistor". Remove it from the item or choose a different set.`; a no-set item accepted any defined dimension incl. one outside the resistor set)*
- [x] Set CRUD works per database, including delete guards while sets are in use. *(36/36 spare-port API checks on a temp database: create/list/update/rename/member-replace/delete blocked with count → cleared refs → delete succeeded; duplicate names rejected case-insensitively; unknown member ids rejected)*
